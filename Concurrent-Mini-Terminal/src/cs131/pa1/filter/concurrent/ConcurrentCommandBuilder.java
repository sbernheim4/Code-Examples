...
public class ConcurrentCommandBuilder {
	...
	private static String adjustCommandToRemoveFinalFilter(String command){
		...
		if(removeRedir.length > 1) {
			//if the redirection does not have an input, then outputLine an error
			if(removeRedir[0].trim().equals("")) {
				System.out.printf(Message.REQUIRES_INPUT.toString(), (">" + removeRedir[1]).trim());
				return null;
			}
			//if redirection is attempted to be piped, outputLine an error
			...
			//if multiple redirections are in the command, outputLine an error
	...
	private static boolean linkFilters(List<ConcurrentFilter> filters, String command){
		...
		String[] cmdlist = command.split("\\|");	//command is brought in so we can outputLine proper error messages
		int cmdindex = 0;
